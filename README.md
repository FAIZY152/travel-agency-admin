...
`Landing -> Login -> Dashboard -> Companies -> Customers -> Document -> QR -> Public Verify`
...
- Milestone 4: Document view with QR generation and print support.
- Milestone 5: Public verification page by QR token.
...
- `companies`
- `customers` (with issue/expiry validation check)
- customer `qr_token` values are generated by the app for document verification
