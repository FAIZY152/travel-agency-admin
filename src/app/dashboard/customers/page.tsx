...
const PAGE_SIZE = 7;
const BASE_URL = "/dashboard/customers";
...
export default async function CustomerListPage(props: {
  ...
  const page = Math.max(1, parseInt(readParam(searchParams.page, "1"), 10) || 1);

  const [{ customers, total, error: customersError }, { companies, error: companiesError }] =
    await Promise.all([
      listCustomersPaginated(page, PAGE_SIZE, query),
      listCompanies(),
    ]);
  ...
  if (query) extraParams.query = query;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          { label: "Total Customers", value: total, color: "#182533" },
          { label: "This Page", value: customers.length, color: "#182533" },
          { label: "Current Page", value: page, color: "#0f766e" },
        ].map((s) => (
      ...
      {isEditing && editCustomer && (
        <article className="panel rounded-[30px] p-6 ring-1 ring-amber-200 shadow-[0_24px_60px_-45px_rgba(200,138,61,0.65)]">
          <div className="flex items-center justify-between gap-3 mb-6">
            ...
            <Link href={BASE_URL} className="secondary-button px-4 py-2.5 text-sm">
              Cancel
            </Link>
          </div>
...
c              </div>
      ...
      <article className="panel rounded-[30px] p-6 ring-1 ring-slate-200 shadow-[0_24px_60px_-45px_rgba(15,23,42,0.35)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <form action={BASE_URL} className="flex w-full flex-wrap items-center gap-3 sm:w-auto">
            <input
              type="search"
              name="query"
              defaultValue={query}
                        ...
                        <tr key={customer.id}>
                          ...
                          <td>
                            <div className="flex flex-wrap justify-end gap-2">
                              ...
                              <Link
                                href={`${BASE_URL}?edit=${customer.id}&page=${page}${query ? `&query=${query}` : ""}`}
